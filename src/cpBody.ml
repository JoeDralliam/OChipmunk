...
struct
  ...
  let is_sleeping = Private.Body.is_sleeping

  let is_static = Private.Body.is_static
    
  let is_rogue = Private.Body.is_rogue

  let kinetic_energy = Private.Body.kinetic_energy

  let eql = Private.Body.eql

  let activate body =
    if not (is_rogue body)
    then begin
      ComponentNode.(body.bnode.idle_time <- 0.);
      SpaceComponent.Component.(activate (root (Some body)))
    end
  ...
  let activate_static body filter =
    ...
    Private.Body.foreach_arbiter body (fun arb ->
      match filter with
        | None -> activate_arbiter arb
        | Some filt when Private.Shape.eql filt arb.aa || Private.Shape.eql filt arb.ab 
            -> activate_arbiter arb
        | _ -> ()
    )

  let push_arbiter body arb =
    Private.assert_soft ((ArbiterThread.for_body arb body).thnext = None) "Internal Error: Dangling contact graph pointers detected. (A)" ;
    Private.assert_soft ((ArbiterThread.for_body arb body).thprev = None) "Internal Error: Dangling contact graph pointers detected. (B)" ;

    let next = body.barbiter_list in
    Private.assert_soft (next = None || (ArbiterThread.for_body CpOption.(!?next) body).thprev = None) "Internal Error: Dangling contact graph pointers detected. (C)" ; 
    (ArbiterThread.for_body arb body).thnext <- next ;

    CpOption.do_if (fun n -> (ArbiterThread.for_body n body).thprev <- Some arb) next ;
  ...
  let sleep_with_group body group =
    ...
    if is_sleeping body
    then assert  SpaceComponent.Component.( 
    ...
    Private.Body.foreach_shape body (fun shape -> Private.Shape.update shape body.bp body.brot) ;
    SpaceComponent.deactivate_body space body ;

    if group <> None
    then ComponentNode.(
      let root = SpaceComponent.Component.root group in
  ...
  let sanity_check body =
    Private.assert_soft (body.bm = body.bm && body.bm_inv = body.bm_inv) "Body's mass is invalid." ;
    Private.assert_soft (body.bi = body.bi && body.bi_inv = body.bi_inv) "Body's moment is invalid." ;
    ...
    Private.assert_soft (body.ba = body.ba && (abs_float body.ba) <> infinity) "Body's angle is invalid." ;    
    Private.assert_soft (body.bw = body.bw && (abs_float body.bw) <> infinity) "Body's angular velocity is invalid." ;
    Private.assert_soft (body.bt = body.bt && (abs_float body.bt) <> infinity) "Body's torque is invalid." ;

    CpVector.assert_sane body.brot "Body's rotation vector is invalid" ;

    Private.assert_soft (body.bv_limit = body.bv_limit) "Body's velocity limit is invalid" ;
    Private.assert_soft (body.bw_limit = body.bw_limit) "Body's angular velocity limit is invalid"
  ...
  let rec filter_constraints n body filter =
    match n with 
      | Some node ->
          if Private.Constraint.eql node filter
          then Private.next_constraint node body
          else begin
            if eql node.ca body
  ...
  let apply_impulse body j r =
    activate body ;
    ConstraintUtils.apply_impulse body j r
  ...
  let each_constraint body func =
    let rec iter list =
      match list with
        | Some constr -> begin
          let next = Private.next_constraint constr body in
          func body constr ;
          iter next
        end
        | None -> ()
  ...
  let each_arbiter body func =
    let rec iter list =
      match list with
        | Some arb -> begin
          let next = Private.next_arbiter arb body in
